...
flowchart TD
    ...
    subgraph P1["Export pass 1 · analyse (no blur)"]
        direction TB
        FRAME --> DET["HeadDetector<br/>PINTO YOLOv9-Wholebody17 (ONNX)<br/>body + head + face boxes, NMS in-graph<br/>optional ±90° rotation assist<br/>(score + area + containment + witness gated)"]
        DET --> SCRFD["ScrfdDetector (fallback)<br/>close-up-scale faces only, when the<br/>primary sees nothing confident"]
        SCRFD --> FUSE{{"fuse_heads()<br/>orphan faces → pseudo-heads"}}
    ...
    REC --> POST["tracklets.postprocess()<br/>trim coasted tails · prune noise ·<br/>verify (cropped re-inference) ·<br/>bridge gaps (corridor + ambiguity gates) ·<br/>interpolate · zero-phase SavGol smooth"]
...
| 2 | Detection | PINTO YOLOv9-Wholebody17 post-ONNX (ONNX Runtime) | [detector.py](../src/libs/detector.py) | One pass → body/head/face boxes. Head class covers all 360° orientations; optional ±90° rotated passes recover sideways heads, gated against hallucinations by score floor, absolute area cap, containment test, and a witness rule (a rotated-only box with no upright corroboration — weak head/face overlap or a body containing its centre — needs a much higher score) |
| 2b | Close-up assist | SCRFD det_10g (ONNX Runtime), fallback only | [scrfd.py](../src/libs/scrfd.py) | When the primary finds no confident head/face (extreme close-up: partial face fills the frame), SCRFD faces are unioned in and grow pseudo-heads — but only at close-up scale (longest side ≥ 25 % of the frame's short side) and above the user's confidence bar, so SCRFD's skin/texture misfires can't seed tracks |
...
| 5 | Offline cleanup | `tracklets.postprocess` (NumPy/SciPy + re-inference) | [tracklets.py](../src/libs/tracklets.py) | Trim coasted tails, prune noise tracklets, **verify** every survivor by re-detecting on magnified crops of its own frames (persistent hallucinations don't reproduce — dropped before render), bridge detection gaps with interpolation (corridor + ambiguity gates against identity smears), zero-phase Savitzky-Golay smoothing |
...
- **Close-up assist:** insightface SCRFD det_10g (~17 MB, bundled), standalone
  ONNX wrapper (no insightface package); overridable via `AVPP_SCRFD*` env vars
