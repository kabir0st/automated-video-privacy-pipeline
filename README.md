...
# 🛡️ Automated Video Privacy Pipeline

**Detect → track → blur.** On-device face anonymization for video,
with a live inspector for tuning every step before you export.
...
<img src="docs/screenshots/hero.png" alt="Inspector — Before, Tracking, and After panels side by side" width="900"/>

</div>

---

Publishing footage shouldn't mean publishing identities. This tool finds every
face in a video, follows it across frames, and paints an irreversible
Gaussian + mosaic blur over it — entirely on your machine. Nothing is uploaded,
ever.

## ✨ Features

- **Three-panel live inspector** — *Before · Tracking · After* side by side, so
...
- **Presets first, sliders when you want them** — pick a preset chip for the
  common cases, or pop open **Advanced ▾** for full control of every
  tunable. Changes re-render the current frame live.
- **Real tracking, not per-frame detection** — InsightFace SCRFD detection +
  106-point landmarks feed a per-face Kalman filter; detections correct it,
...
- **Stackable blur layers** — add, remove and reorder Gaussian and pixelate
  passes to taste; the default is the classic Gaussian → mosaic combo.
...
- **GPU where it helps** — ONNX Runtime picks DirectML → CUDA → CPU
...
| **Pose Assist** | on | — | Track the head via body pose when the face detector loses it (head turns, looking down), keeping the track corrected indefinitely while the person is visible. |
...
| **Layer Stack** | Gaussian 71 → Pixelate 10 | up to 5 layers | Ordered blur passes applied in sequence. Gaussian strength is the kernel size (3–151, odd); Pixelate strength is the mosaic cell size (2–40). Stack more layers for harder anonymization. |
...
- **Hoodies / hair still visible?** Hair Extra `1.5`+, then Hull Expand. The
...
src/
  ...
  libs/
    ...
    pose_head.py     MediaPipe pose → head boxes for lost-track revival
